...
export default function Header() {
  ...
  return (
    // CHANGED: Swapped 'sticky top-4' out for an explicit fixed overlay container
    <header className="fixed top-0 left-0 w-full z-50 pt-4 px-4 sm:px-6 lg:px-8 pointer-events-none">
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-2">
                  <img src="/shotbyvor-logo.png" alt="ShotbyVOR" className="h-8 w-auto" />
                  <span className="font-bold text-slate-800 tracking-wide">ShotbyVOR</span>
                </div>
                <button onClick={() => setIsMenuOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:text-violet-600 transition">
                  ...
                  return (
                    <Link
                      ...
                      className={`px-4 py-3 rounded-xl text-sm font-semibold tracking-wide transition ${
                        active ? "bg-violet-600 text-white" : "text-slate-700 hover:bg-violet-50 hover:text-violet-600"
                      }`}
                    >
                      {link.label}
                    </Link>
